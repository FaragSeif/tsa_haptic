...
class device_states_t(object):
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(device_states_t._get_packed_fingerprint())
    ...
    def decode(data):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != device_states_t._get_packed_fingerprint():
            raise ValueError("Decode error")
        return device_states_t._decode_one(buf)
    decode = staticmethod(decode)

    def _decode_one(buf):
        self = device_states_t()
    ...
    def _get_hash_recursive(parents):
        if device_states_t in parents: return 0
    ...
    def _get_packed_fingerprint():
        if device_states_t._packed_fingerprint is None:
            device_states_t._packed_fingerprint = struct.pack(">Q", device_states_t._get_hash_recursive([]))
        return device_states_t._packed_fingerprint
    _get_packed_fingerprint = staticmethod(_get_packed_fingerprint)

    def get_hash(self):
        """Get the LCM hash of the struct"""
        return struct.unpack(">Q", device_states_t._get_packed_fingerprint())[0]
