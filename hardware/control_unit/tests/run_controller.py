...
try:
    ...
    while True:
        ...
        # if rfds:
        lc.handle()
        # print(device_states.timestamp, device_states.motor_angles)

        # ///////////////////////
        # QP CARTESIAN CONTROLLER
        # ///////////////////////
        print(int(1000*t))
        if commands.arm:
            ...
            if commands.mode == 11:
                ...
                joint_pd = kp@(- theta) + kd@(- dtheta)  # + 3*np.ones(3)
                control[:3] = joint_pd
                control[3] = 50
            ...
            if commands.mode == 12:
                ...
                cartesian_position, jacobian_m, jacobian_d, jacobian_s = full_kinematics(
                    device_states.cartesian_position, device_states.motor_angles)
                cartesian_velocity = np.linalg.pinv(
                    jacobian_d) @ device_states.motor_speeds

                cartesian_pd = cartesian_stiffnes@(commands.desired_position[:3] - device_states.cartesian_position) + \
                    cartesian_damping @ (
                        commands.desired_speed[:3] - device_states.cartesian_velocity)
                control = get_torques(
                    jacobian_d, jacobian_s, cartesian_pd, pretension=1)
