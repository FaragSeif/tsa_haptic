    ...
    while i<=N:
        ...
        if rfds:
            ...
            if t - ts >= 1/UPDATE_RATE:
                ...
                cartesian_velocity = np.linalg.pinv(jacobian_d) @ device_states.motor_speeds
                
                tension = np.zeros(4)
                tension[:3] = np.array(device_states.tensions[:3])
                tension[3] = device_states.tensions[3]
                # print(device_states.tensions[3])
                cartesian_force = jacobian_m.T @ tension
                print(cartesian_position)
                # print(cartesian_force)
                # print(device_states.carriage_positions )
                # # print(device_states.motor_torques )
